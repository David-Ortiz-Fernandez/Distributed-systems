...
//---------------------------------------------------------------------------------------------------------
//Parte de Threads MOSTRAR--------------------------------------------------------------------------------		
		int * socketd;

                socketd =args;

		printf("Thread with ID %ld working...\n", pthread_self());
		sleep(3);
		printf("Thread with ID %ld Done!\n",  pthread_self());
...
//Parte de Sockets----------------------------------------------------------------------------------------------
		...
		socketPlayer1 = accept(*socketd, (struct sockaddr *) &player1Address, &clientLength);
		...
		socketPlayer2 = accept(*socketd, (struct sockaddr *) &player2Address, &clientLength);
...
//Variables de Threads-------------------------------------------------------------------------------------------------------------------------	
	pthread_t threadVector[3];
	t_threadData threadDataVector [3];
...
//-------------------------------------------------------------------------------------------------
		// Create threads...
		for(currentThread=0; currentThread<3; currentThread++){

			pthread_create(&threadVector[currentThread], NULL, threadTaski, (void *)&socketfd);
		}
